...
jQuery(document).ready(function ($) {
  ...
  $('ul.slideshow').wrap('<div class="slideshow-wrap"><div class="slideshow-inner"></div></div>').each(function () {
    ...
    // stop play button
    wrap.find('.slideshow-buttons').append('<li class="slideshow-stop"><a href="#slideshow-stop">Stop</a></li>');
  ...
  $('img.style1, img.style2, img.style3').each(function () {
    $(this).wrap('<span>');
    $(this).parent('span').attr('class', 'img-wrap ' + $(this).attr('class')).css('background-image', 'url(' + $(this).attr('src') + ')').css('background-position', 'center center').css('background-repeat', 'no-repeat').css('height', $(this).height()).css('width', $(this).width());
  ...
  $('img.caption').each(function () {
    ...
    $(this).parents('div.caption').attr('class', 'caption ' + $(this).attr('class')).css('width', $(this).width() + 'px');
    if ($(this).attr('title')) {
...
(function (g) {
  ...
  g.fn.calendarWidget = function (a) {
    ...
    a = "" + ('<h4 id="current-month">' + "January,February,March,April,May,June,July,August,September,October,November,December".split(",")[month] + " " + year + "</h4>");
    ...
    c = 0 == c && b ? 7 : c;
    for (j = b = 0; 42 > j; j++) j < c ? a += '<td class="other-month"><span class="day">' + (e - c + j + 1) + "</span></td>" : j >= c + f(month, year) ? (b += 1,a += '<td class="other-month"><span class="day">' + b + "</span></td>") : a += '<td class="current-month day' + (j - c + 1) + '"><span class="day">' + (j - c + 1) + "</span></td>",6 == j % 7 && (a += "</tr>");
    this.html(a + "</table>");
    return this
  }
})(jQuery);